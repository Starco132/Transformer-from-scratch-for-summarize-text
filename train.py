...
def get_all_sentences(ds):
    for item in ds:
        yield item["content"] + item["summary"]
...
def get_ds(config):
    # Load the dataset
    all_files = [f for f in os.listdir(config["datasource"]) if f.endswith(".csv")]
    print(all_files)
    ds_raw = pd.DataFrame()
    for file in all_files:
        file_path = os.path.join(config["datasource"], file)
        try:
            df = pd.read_csv(file_path)
            if {"content", "summary"}.issubset(df.columns):
                ds_raw = pd.concat(
                    [ds_raw, df[["content", "summary"]]], axis=0, ignore_index=True
                )
        except Exception as e:
            print(f"Không thể đọc file {file_path}: {e}")
    ds_raw.info()
    if config["word_seg"]:
        ds_raw.loc[:, "Content"] = ds_raw["Content"].apply(ViTokenizer.tokenize)
        ds_raw.loc[:, "Summary"] = ds_raw["Summary"].apply(ViTokenizer.tokenize)
    # ds_raw = pd.read_csv(config["datasource"])
    ...
    for i, item in enumerate(train_ds_raw):
        src_ids = tokenizer.encode(item["content"]).ids
